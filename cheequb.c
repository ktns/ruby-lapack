...
rb_cheequb(int argc, VALUE *argv, VALUE self){
  ...
  work.r = (real)NUM2DBL(rb_funcall(rb_work, rb_intern("real"), 0));
  work.i = (real)NUM2DBL(rb_funcall(rb_work, rb_intern("imag"), 0));
  if (!NA_IsNArray(rb_a))
