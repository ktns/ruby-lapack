...
rb_clatzm(int argc, VALUE *argv, VALUE self){
  ...
  if (!NA_IsNArray(rb_c2))
    rb_raise(rb_eArgError, "c2 (7th argument) must be NArray");
  if (NA_RANK(rb_c2) != 2)
    rb_raise(rb_eArgError, "rank of c2 (7th argument) must be %d", 2);
  ldc = NA_SHAPE0(rb_c2);
  ...
  if (!NA_IsNArray(rb_c1))
    rb_raise(rb_eArgError, "c1 (8th argument) must be NArray");
  if (NA_RANK(rb_c1) != 2)
    rb_raise(rb_eArgError, "rank of c1 (8th argument) must be %d", 2);
