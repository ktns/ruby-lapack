...
extern VOID zla_gbrcond_x_(doublereal *__out__, char *trans, integer *n, integer *kl, integer *ku, doublecomplex *ab, integer *ldab, doublecomplex *afb, integer *ldafb, integer *ipiv, doublecomplex *x, integer *info, doublecomplex *work, doublereal *rwork);
static VALUE
rb_zla_gbrcond_x(int argc, VALUE *argv, VALUE self){
  ...
  if (NA_SHAPE0(rb_x) != n)
    rb_raise(rb_eRuntimeError, "shape 0 of x must be the same as shape 1 of ab");
  ...
  zla_gbrcond_x_(&__out__, &trans, &n, &kl, &ku, ab, &ldab, afb, &ldafb, ipiv, x, &info, work, rwork);
